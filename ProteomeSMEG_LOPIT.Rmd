...
par(mfrow = c(1, 2))
plot2D(hl, main = "Known markers", col = col)
legend(-8, -4, unique(markers), lty = 1, col = col, bty = "n")
plot2D(hl, dims = c(1, 3), main = "Known markers", col = col)
dev.off()


#hl2 <- addMarkers(hl2, markers2)
# par(mfrow = c(1, 2))
# plot2D(hl2, main = "pRolocmarkers", col = getStockcol()[c(1,4,2,7,3,11,10,9)])
# addLegend(hl2, cex = .6, col = getStockcol()[c(1,4,2,7,3,11,10,9)])
# plot2D(hl2, dims = c(1, 3), main = "Marker resolution along PC 1 and 7", col = getStockcol()[c(1,4,2,7,3,11,10,9)])
# plot3D(hl2)

# par(xpd=TRUE, mar = c(4,4,4,16), mfrow = c(1,1))
# hlo <- hl2[, order(hl2$Sample.name)]
