...
```{r, echo=T}
load("Data/Proline/Data00.Rdata")
```


# Data normalisation 

1/ Log2-transform the matrix was performed in the "ProteomeSMEG_DataDescription_Proline.Rmd" file together with normalisation to median.
...
```{r}
# Make export table:
colnames(mat) <- paste0(colnames(mat), "_Raw")
export <- cbind(input, mat)
colnames(matlog) <- paste0(colnames(matlog), "_Log2Norm")
...
```{r}
boxplot(matlog, las = 2, main = "Log2-transformed intensities after normalisation", cex = 0.4, cex.axis = 0.5)
rm(matlog)
```
...
```{r}
colnames(mat) <- sub("_[1234]_Log2Norm", "", colnames(mat))
...
The data is exported as "Output/Proline/ExportedTableBeforeStat.txt"
```{r}
write.table(export, row.names = F, "Output/Proline/ExportedTableBeforeStat.txt", sep = "\t")
save(list = c("export", "mapping", "mat", "matcol"), file = "Data/Proline/Data01.Rdata")
```
